...
// things to set!  can be read in with parameter file
	...
	meananom[ip] =0.0;    // initial 
        ...
        fgets(line,300,fpi);  sscanf(line,"%lf %lf",&kfac, &gammafac2);       // change inside Rshell  by this factor
        fgets(line,300,fpi);  sscanf(line,"%lf",&Rshell);     // Rshell    
...
// hard/soft interior!
   // adjust_ks_abc_fac(r, 0, kfac, 0.0, 0.0,  0, 0, Rshell, Rshell, Rshell, 0,0, 0, 1);
   printf("Rshell = %.2f\n",Rshell);

   adjust_ks(r, 0, kfac*ks, gammafac2*spring_mush.gamma, 0.0, 0.0, Rshell);
...
void heartbeat(struct reb_simulation* const r){
	...
	if (reb_output_check(r,10.0*r->dt)){
        ...
        if (fabs(r->t - t_damp) < 0.9*r->dt) { // set_gamma(r,gamma_all); 
            set_gamma_fac(r,gamma_fac);
        }
